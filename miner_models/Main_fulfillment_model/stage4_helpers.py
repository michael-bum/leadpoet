...
# Load JSON config files from same directory
VALIDATOR_PATH = Path(__file__).parent
GATEWAY_UTILS_PATH = VALIDATOR_PATH
...
def _call_llm(prompt: str, openrouter_api_key: str, model: str = 'google/gemini-2.5-flash-lite') -> Dict[str, Any]:
        ...
        if resp.status_code == 200:
            ...
            if json_match:
                try:
                    ...
                    return {'success': True, 'parsed': parsed, 'raw': content}
                except:
                    pass
...
def validate_role_with_llm(
    ...
    """
    Validate role using two-stage LLM approach.

    Call 1 (Slug check): Does the LinkedIn profile snippet mention a role?
      - If role visible + matches claimed → PASS (done)
      - If role visible + mismatch → FAIL (done, no call 2)
      - If no role visible → proceed to call 2

    Call 2 (Fallback): Check pub/dir + other results for role evidence.
      - Only runs when call 1 finds no role in slug
    ...
    """
    # ========== CALL 1: Slug check ==========
    slug_prompt = f'''You are a strict job title verifier. Check if the LinkedIn profile result mentions a job title/role for this person.
...
1. Does the LinkedIn snippet show a job title/role for this person? (not a description, not volunteer work, not a LinkedIn post — only their actual title/headline)
2. If yes, what is the exact role shown?
3. Does it match the claimed role "{claimed_role}"?
...
- The role must appear as an EXACT title phrase, not scattered words in a description. "Experienced engineer in mechanical design" is NOT "Mechanical Engineer".
- Word order variations OK ("Distribution Center and Transportation Admin" = "Transportation and Distribution Center Admin")
- C-suite titles NOT interchangeable (CEO≠CTO≠CFO≠CIO≠CISO≠CCO≠CMO)
- Seniority down OK: "Senior Software Engineer" on LinkedIn matches claimed "Software Engineer". But claiming HIGHER than actual is NOT OK.
- Specialization prefixes are NOT OK: "Embedded Software Engineer" ≠ "Software Engineer", "Structural Analysis Engineer" ≠ "Aerospace Engineer", "R&D Software Engineer" ≠ "Software Engineer", "Radar Systems Engineer" ≠ "Systems Engineer". If the actual title has a domain/specialization word before the claimed role, it's a different role.
- Compound roles: if title has "and"/"&" (e.g., "Principal and Client Advisor"), claimed can match either component.
- Volunteer/community roles in About section are NOT professional titles.

JSON only: {{"role_visible": bool, "role_found": "", "role_pass": bool}}'''

    call1 = _call_llm(slug_prompt, openrouter_api_key, model)

    if not call1.get('success'):
        return {
            'success': False,
            'error': call1.get('error', 'unknown'),
            'raw': call1.get('raw', '')
        }

    parsed1 = call1['parsed']
    role_visible = parsed1.get('role_visible', False)
    role_found = parsed1.get('role_found', '')
    role_pass = parsed1.get('role_pass', False)

    # If slug has a role → decision is final
    if role_visible:
        return {
            'success': True,
            'role_pass': role_pass,
    ...
    # ========== CALL 2: Fallback (only if slug has no role) ==========
    ...
    fallback_prompt = f'''You are a strict job title verifier. The person's LinkedIn profile did NOT show their job title. Check these other sources.
...
3. Check LINKEDIN DIRECTORY first — these show LinkedIn profile headlines.
   - Be suspicious of pipe "|" separated headlines (keyword taglines, not real titles)
   - "Student", "Intern", "Seeking", "Aspiring", "Open to" are NOT professional titles
   - Only trust clean, specific job titles
4. If no directory match, check OTHER RESULTS that mention BOTH "{name}" AND "{company}".
5. Matching rules:
   - Abbreviations OK, NO synonyms
   - Exact title phrase required (not scattered words)
   - Seniority down OK (Senior X matches claimed X), claiming higher NOT OK
   - Specialization prefixes NOT OK: "Embedded Software Engineer" ≠ "Software Engineer", "Structural Analysis Engineer" ≠ "Aerospace Engineer", "R&D Software Engineer" ≠ "Software Engineer". If the actual title has a domain/specialization word before the claimed role, it's a different role.
   - Compound roles: claimed can match either component
   - C-suite NOT interchangeable

JSON only: {{"role_pass": bool, "role_found": ""}}'''

    call2 = _call_llm(fallback_prompt, openrouter_api_key, model)

    if not call2.get('success'):
        return {
            'success': False,
            'error': call2.get('error', 'unknown'),
            'raw': call2.get('raw', '')
        }

    parsed2 = call2['parsed']
    return {
        'success': True,
        'role_pass': parsed2.get('role_pass', False),
        'role_found': parsed2.get('role_found', ''),
        'raw': call2.get('raw', ''),
        'method': 'llm_fallback'
    }
