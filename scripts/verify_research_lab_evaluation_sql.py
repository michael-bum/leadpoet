...
GATEWAY_SCORING_MIGRATION = ROOT / "scripts" / "36-research-lab-gateway-scoring-audit.sql"
...
GATEWAY_SCORING_REQUIRED_TABLES = (
...
GATEWAY_SCORING_REQUIRED_VIEWS = (
...
def main() -> int:
    ...
    gateway_scoring_sql = GATEWAY_SCORING_MIGRATION.read_text(encoding="utf-8")
    errors = verify_sql(sql)
    errors.extend(verify_candidate_sql(candidate_sql))
    errors.extend(verify_auto_loop_sql(auto_loop_sql))
    errors.extend(verify_gateway_scoring_sql(gateway_scoring_sql))
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print(
        ...
        "candidate queue tables, auto-research loop events, gateway-owned scoring/audit tables, current views, service-role RLS, append-only triggers, no fulfillment writes."
    )
    return 0
...
def verify_gateway_scoring_sql(sql: str) -> list[str]:
    errors: list[str] = []
    lowered = sql.lower()
    if "begin;" not in lowered or "commit;" not in lowered:
        errors.append("gateway scoring migration must be wrapped in BEGIN/COMMIT")
    if "gateway-owned" not in lowered or "audit bundles" not in lowered:
        errors.append("gateway scoring migration must document gateway-owned scoring and audit bundles")
    if re.search(r"\b(?:CREATE|ALTER|DROP)\s+TABLE\b[^;]*\bfulfillment_", sql, re.IGNORECASE):
        errors.append("gateway scoring migration must not modify fulfillment tables")
    if FORBIDDEN_GRANT_RE.search(sql):
        errors.append("gateway scoring migration must not grant privileges to anon/authenticated")
    if re.search(r"GRANT\s+[^;]*(UPDATE|DELETE)[^;]*TO\s+service_role", sql, re.IGNORECASE):
        errors.append("append-only gateway scoring tables must not grant UPDATE/DELETE to service_role")

    for table in GATEWAY_SCORING_REQUIRED_TABLES:
        _require_table(sql, table, errors)
    for view in GATEWAY_SCORING_REQUIRED_VIEWS:
        _require_view(sql, view, errors)
    ...
    for marker in (
        ...
        if marker not in sql:
            errors.append(f"gateway scoring migration missing marker: {marker}")
    return errors
