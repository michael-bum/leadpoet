-- Research Lab: age stale paused runs out of queue capacity checks.
--
-- Deployment policy:
--   * Apply after scripts 43 and 44.
--   * Keeps paused runs active while fresh, but prevents abandoned paused rows
--     from consuming one-hotkey/global capacity forever after maintenance.
...
COMMENT ON FUNCTION public.guard_research_lab_queue_capacity() IS
    'Serializes Research Lab queued event admission and rejects over-capacity or same-hotkey active runs; stale paused rows age out by active_loop_stale_after_seconds.';
