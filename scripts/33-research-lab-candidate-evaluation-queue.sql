-- Research Lab Phase 1: validator-scored candidate evaluation queue.
--
-- Deployment policy:
--   * Apply after scripts 27, 28, 29, 30, 31, and 32.
--   * Gateway hosted workers create candidate artifacts only.
--   * Validator qualification workers evaluate these candidates against the
--     active private ICP set and submit official score bundles.
...
COMMENT ON TABLE public.research_lab_candidate_artifacts IS
    'Append-only Research Lab candidate patch artifacts generated by gateway hosted workers. Validators score these against private Supabase ICP sets.';
COMMENT ON TABLE public.research_lab_candidate_evaluation_events IS
    'Append-only validator evaluation lifecycle events for Research Lab candidates.';
COMMENT ON VIEW public.research_lab_candidate_evaluation_current IS
    'Current validator evaluation status projection for Research Lab candidates.';
